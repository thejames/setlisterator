...
**Auditioning** (playing a candidate in the browser to check it's the song you expect — see the glossary) is a read-only side-path that touches none of the above. `audition_source(config, rating_key, force_transcode, offset)` turns a rating key into a tokenized Plex URL plus the `mode` it chose, and the pure `audition_mode(codec, force_transcode)` is the whole policy: browser-decodable codecs stream the original file, everything else transcodes at `AUDITION_MAX_BITRATE`. `web.py` adds `/audition/<key>` (JSON) and `/audition/<key>/stream` (a `Range`-forwarding proxy), and composes the proxy URL itself — core stays free of Flask routes. Which URL the client uses is decided by a once-per-tab reachability probe, because only the browser knows whether it can reach Plex; the proxy exists for when it can't. `AUDITION_ALWAYS_TRANSCODE` (default on, `AUDITION_TRANSCODE_DEFAULT`) is overridable per browser from the gear menu, so the client sends its choice on every request and the server stays stateless — including on `stream_path`, or the two routes can disagree about the mode. Best-effort like the poster: a failure warns inline and never touches a selection. See `docs/adr/0004-audition-streams-through-the-app.md`.
...
**Which version of a song you get** is decided by candidate order, in three layers: an explicitly named **preferred album** (`_prefer_album_first`, the only thing that outranks tier), then title tier, then the **earliest album** — `_ranked_matches` breaks same-tier ties on album year ascending, so the oldest-dated album carrying the song is the default. Year sorts strictly *within* a tier, never across one, which is also what keeps a medley from being floated. Undated albums sort last (`_YEAR_UNKNOWN`); a year of `0` or less counts as undated. The year comes off tracks already fetched via `_track_year` — Plex puts `parentYear` on every `<Track>` on both matcher endpoints, but plexapi 4.18.1 doesn't map it, so the helper probes the attribute, then the raw element's `attrib`, then `year`, and never raises. No album is auto-preferred: `_album_coverage` still ranks albums by how much of the setlist they cover, but only to order the preview dropdown. See `docs/adr/0006-earliest-album-is-the-default-version.md`.
